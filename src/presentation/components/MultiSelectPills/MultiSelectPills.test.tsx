...
describe("SingleSelectPills", () => {
  ...
  it("renders all options", () => {
    const { getByText } = render(<SingleSelectPills options={options} selected="" onChange={jest.fn()} />);
  ...
  it("highlights the selected option", () => {
    ...
    const { getByText } = render(<SingleSelectPills options={options} selected={selected} onChange={jest.fn()} />);
    const selectedPill = getByText(selected).parent;
    expect(selectedPill?.props.style).toEqual(
      expect.objectContaining({ color: "rgb(248, 249, 250)", fontSize: 16, fontWeight: "400" }),
    );
  });

  it("calls onChange with correct label when an option is pressed", () => {
    const onChange = jest.fn();
    const { getByText } = render(<SingleSelectPills options={options} selected="" onChange={onChange} />);
  ...
  it("does not call onChange when disabled", () => {
    const onChange = jest.fn();
    const { getByText } = render(<SingleSelectPills options={options} selected="" onChange={onChange} disabled />);
    fireEvent.press(getByText("Option 1"));
    expect(onChange).not.toHaveBeenCalled();
  });

  it("renders with empty options", () => {
    const { toJSON } = render(<SingleSelectPills options={[]} selected="" onChange={jest.fn()} />);
    expect(toJSON()).toMatchSnapshot();
  });

  it("applies correct text color for selected and unselected pills", () => {
    const { getByText } = render(<SingleSelectPills options={options} selected="Option 1" onChange={jest.fn()} />);
