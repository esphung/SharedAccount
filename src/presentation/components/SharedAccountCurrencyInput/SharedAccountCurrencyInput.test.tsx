...
describe("SharedAccountCurrencyInput", () => {
	it("renders with default value (0 cents)", () => {
		const { getByDisplayValue } = render(
			<SharedAccountCurrencyInput value={0} onChangeValue={jest.fn()} />
		);
	...
	it("calls onChangeValue with correct cents when input changes", () => {
		...
		const { getByDisplayValue } = render(
			<SharedAccountCurrencyInput value={0} onChangeValue={onChangeValue} />
		);
	...
	it("applies custom style", () => {
		...
		expect(StyleSheet.flatten(input.props.style)).toEqual(
			expect.objectContaining({ backgroundColor: "pink" })
		);
	});
	...
	it("keeps cursor at the end after value change", () => {
		const { getByDisplayValue } = render(
			<SharedAccountCurrencyInput value={1234} onChangeValue={jest.fn()} />
		);
