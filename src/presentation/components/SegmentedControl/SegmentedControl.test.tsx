...
describe("SegmentedControl", () => {
  ...
  it("applies selected style to the correct option", () => {
    const { getAllByText } = render(<SegmentedControl options={options} selectedIndex={2} onSelect={onSelect} />);
  ...
  it("applies custom containerStyle", () => {
    ...
    expect(flatList.props.contentContainerStyle).toEqual(
      expect.arrayContaining([expect.objectContaining({ alignItems: "center" }), expect.objectContaining(customStyle)]),
    );
  });
});
