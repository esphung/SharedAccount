...
const calculateMonthlyTotoal = (type: "credit" | "expense", arr: Transaction[]) => {
  ...
  return arr
    ...
    .filter(
      (item) =>
        item.date.getTime() >= DateTime.now().startOf("month").toJSDate().getTime() &&
        item.date.getTime() < lastEndOfMonth.toJSDate().getTime(),
    )
    .reduce((sum, item) => sum + item.amount, 0);
};

const mapTransactionsToRange = (transactions: Transaction[], type: "credit" | "expense") => {
  ...
  return range.map((day) => {
    return filtered
      .filter((item) => {
        ...
        return date.day === day && date.month === DateTime.now().month && date.year === DateTime.now().year;
      })
      .reduce((sum, item) => sum + item.amount, 0);
  });
};

const SpendingStats = ({ transactions = [] }: { transactions: Transaction[] }) => {
  const expenseTransactionsRange = React.useMemo(() => mapTransactionsToRange(transactions, "expense"), [transactions]);
  ...
  const renderTopLabelComponent = (value: number) => (
    <SharedAccountText style={styles.label}>
      {Math.round(Number(MoneyFunctions.formatMoney(Number(value)).replace("$", "")))}
    </SharedAccountText>
  );
  ...
  const chartData: {
    ...
    const zipped = temp.map((item, index) => {
      ...
      return {
        stacks,
        topLabelComponent: () => renderTopLabelComponent(creditAmount > expenseAmount ? creditAmount : expenseAmount),
      };
    });
  ...
  const lineChartData: {
    ...
    const lineData = [
      ...expenseTransactionsRange.map((value, index) => ({
        pos: index,
        value,
        dataPointText: value > 0 ? `${MoneyFunctions.formatMoney(value, 0)}` : "",
      })),
    ];
